...
function checkGuess() {
    ...
    if (guess.length !== checkGuessLength.size || guess.length !== 4) {
        document.getElementById("logsArea").value += `${guess} est invalide, veuillez entrer un nb composé de exactement 4 chiffres différents.\n`;
        return null;
    }
...
function printRules() {
    alert("Entrez un nb composé de 4 chiffres différents dans la case à coté de 'Guess'. L'ordinateur le compare avec le code secret et vous donne deux indices: les numéros 'bulls' (B) et des 'cows (C). Qu'est-ce que cela signifie ? Un 'bulls' est un chiffre qui est présent dans les deux codes à la meme positon. Un 'cows' est un chiffre qui est présent dans les deux codes mais à une position différente. Par exemple, si le code secret est 7512 et que vous essayez 5392, la réponse sera '1B 1C' (1 bull 1cow). C'est tout! ")
}